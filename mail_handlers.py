"""Python 3 compatibility shim for inbound mail handling.

``google.appengine.ext.webapp.mail_handlers`` does not exist on the python3
runtime, so ``InboundMailHandler`` is reimplemented on top of webapp2 and
``google.appengine.api.mail`` (which still provides ``InboundEmailMessage``
under bundled services). Subclasses override ``receive(mail_message)`` exactly
as before and register via ``mapping()``.
"""
import webapp2
